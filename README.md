...
1. [3分で始める（友人・ご家族向け・一番かんたん）](#3分で始める友人ご家族向け一番かんたん)
2. [できること](#できること)
3. [アーキテクチャ](#アーキテクチャ)
4. [ディレクトリ構成](#ディレクトリ構成)
5. [動作要件](#動作要件)
6. [Macでのセットアップ手順（Claude Codeを使う場合）](#macでのセットアップ手順claude-codeを使う場合)
7. [起動方法](#起動方法)
8. [使い方（BGM生成の流れ）](#使い方bgm生成の流れ)
9. [API設定方法（外部AI音楽生成サービスを使う場合）](#api設定方法外部ai音楽生成サービスを使う場合)
10. [WAV / MP3 / FLAC 書き出し方法](#wav--mp3--flac-書き出し方法)
11. [バックエンドAPI一覧](#バックエンドapi一覧)
12. [テストの実行方法](#テストの実行方法)
13. [トラブルシューティング](#トラブルシューティング)
14. [セキュリティ・著作権について](#セキュリティ著作権について)
15. [既知の制限・今後の拡張](#既知の制限今後の拡張)

---

## 3分で始める（友人・ご家族向け・一番かんたん）

ターミナル操作に慣れていない方は、この方法が一番簡単です。

### Macの場合

1. 事前にこれだけインストールしてください（一度だけ）：
   - [Python](https://www.python.org/downloads/)（3.11以上）
   - [Node.js](https://nodejs.org/)（18以上、LTS版でOK）
   - ffmpeg：ターミナルで `brew install ffmpeg`（[Homebrew](https://brew.sh/)が必要）
     - MP3書き出しをしないなら省略可（WAV/FLACは使えます）
2. このプロジェクトのフォルダを開き、**`start.command` をダブルクリック**します。
3. 初回はセットアップのため数分かかります。「Macは開発元を確認できないため開けません」と出たら、`start.command` を **右クリック→「開く」** を選び直してください（初回のみ）。
4. 自動的にブラウザが開き、アプリが使える状態になります。

### Windowsの場合

1. 事前にこれだけインストールしてください（一度だけ）：
   - [Python](https://www.python.org/downloads/)（インストール時に「Add python.exe to PATH」に必ずチェック）
   - [Node.js](https://nodejs.org/)（LTS版）
   - ffmpeg：`winget install ffmpeg`（省略するとMP3書き出しのみ使えません）
2. このプロジェクトのフォルダを開き、**`start.bat` をダブルクリック**します。
3. 「WindowsによってPCが保護されました」と出た場合は「詳細情報」→「実行」を選んでください（初回のみ）。
4. 自動的にブラウザが開き、アプリが使える状態になります。
5. 起動後にまず「どなたが使いますか？」という画面が出ます。ご自分の名前を選ぶか、初めての方は「＋ 新しいプロフィールを追加」から名前とアイコンを登録してください（パスワードは不要です）。

### ご家族みんなで使うには

- **プロフィール分け**：起動時に選ぶプロフィールごとに「保存したBGM」が分かれます。保存済みBGM一覧では「自分の作品のみ」「家族みんなの作品」を切り替えて見られ、お気に入り（★）も付けられます。
- **同じ家の中の他のスマホ・タブレットから使う**：`start.command` / `start.bat` を起動したパソコンと同じWi-Fiに接続していれば、起動時にターミナル（またはコマンドプロンプト）に表示される「ご家族の他のスマホ・タブレットからも…」の行に出るURL（例：`http://192.168.1.5:5173`）を、他の端末のブラウザで開くだけで使えます。追加のインストールは不要です。
  - このURLはご家庭内のネットワークの中だけで使えるもので、インターネットに公開されるものではありません。

### 終了するには

開いたウィンドウを閉じるか、ウィンドウ内で `Ctrl+C` を押してください。バックエンド・フロントエンドの両方が自動的に終了します。

> これらのスクリプトは、初回のみ自動でセットアップ（仮想環境作成・依存関係インストール）を行い、2回目以降は数秒でアプリが起動します。手動でのセットアップ手順を知りたい方や、開発者の方は以降の章を参照してください。
...
- 家族プロフィール分け（パスワード不要・名前とアイコンだけの簡易プロフィール）、作品ごとの作成者表示・お気に入り登録
- 同じ家庭内LANの他のスマホ・タブレットからもアクセス可能
...
│   │   ├── storage/                # SQLite (プロジェクト保存・家族プロフィール)
...
│   └── vite.config.ts
├── scripts/
│   ├── start.sh                    # Mac/Linux用 起動ロジック（start.commandから呼ばれる）
│   └── start.ps1                   # Windows用 起動ロジック（start.batから呼ばれる）
├── start.command                   # Mac: ダブルクリックで起動
├── start.bat                       # Windows: ダブルクリックで起動
├── .gitignore
└── README.md
```
...
| POST | `/api/projects` | プロジェクト保存（`profile_id` を指定すると作成者として紐づく） |
| GET | `/api/projects` | プロジェクト一覧（`profile_id` クエリで絞り込み可） |
| GET | `/api/projects/{id}` | プロジェクト取得 |
| DELETE | `/api/projects/{id}` | プロジェクト削除 |
| POST | `/api/projects/{id}/favorite` | プロジェクトのお気に入り登録・解除 |
| GET | `/api/profiles` | 家族プロフィール一覧 |
| POST | `/api/profiles` | 家族プロフィール作成（名前・アイコン） |
| DELETE | `/api/profiles/{id}` | 家族プロフィール削除 |
...
| 120分のBGM生成に時間がかかる | 実測で合成・音響処理・分析・書き出しを合わせて約15〜20分、ピークメモリ約13GB程度かかります（開発機での実測値。環境により変動します）。生成はサーバー側の別スレッドで行われるため、待っている間もアプリの他の操作はブロックされません。すぐに試したい場合はまず5〜15分で動作確認してください。 |
...
| 家族の他の端末からアクセスできない | パソコンと同じWi-Fi（家庭内LAN）に接続しているか確認してください。Macの場合は初回起動時に「ネットワーク上の着信接続を許可しますか」と聞かれたら「許可」を選んでください。Windowsの場合はファイアウォールの確認ダイアログで「プライベートネットワーク」へのアクセスを許可してください。 |
...
- 120分の長時間BGMは、生成・音響処理・分析・書き出しをまとめて実際に完走することを確認済みです（実測: 約15〜20分、ピークメモリ約13GB）。メモリに余裕がない環境では、まず短い時間で試してから長時間BGMに進むことをおすすめします。
- `elevenlabs` / `stability` アダプタは `MusicProvider` インターフェースの実装例です。各社の実際のAPI仕様に応じて調整してください。また、これらの外部サービスへの実際の疎通確認はご自身の環境・APIキーで行ってください（サンドボックス環境ではネットワークポリシーにより外部APIへ到達できない場合があります）。
