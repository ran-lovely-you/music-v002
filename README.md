...
事前インストールは不要です。Python / Node.js / ffmpeg が入っていなくても、`start.command` の初回実行時に自動でインストールされます（[Homebrew](https://brew.sh/)がなければそれも自動導入します。途中でMacのログインパスワードを聞かれることがあります）。

1. （任意・推奨・一度だけ）Gatekeeperの警告を先に消しておきたい場合は、ターミナルを開き `xattr -dr com.apple.quarantine ` と入力したあと、プロジェクトフォルダをターミナルの画面にドラッグ＆ドロップしてパスを入力し、Enterキーを押してください。これで以後は右クリック不要の通常のダブルクリックで起動できます。
   - この手順を省略しても構いません。省略した場合は手順3で一度だけ「右クリック→開く」が必要になります。
...
> **なぜ「開発元を確認できません」が出るの？** ネットからダウンロードしたファイルにMacが自動で付ける安全機能（Gatekeeper）によるものです。この警告自体を完全になくすには、開発者側でApple Developer Program（有料・年間$99）に登録し、配布物にコード署名・公証（notarization）を行う必要があります。それには当方のApple IDでの登録・証明書発行が前提となるため、このリポジトリだけでは対応できません。上記1の手順で手間を1回のコマンドに減らすのが、無料でできる最も簡単な回避策です。

### Windowsの場合

事前インストールは不要です。Python / Node.js / ffmpeg が入っていなくても、`start.bat` の初回実行時に [winget](https://learn.microsoft.com/ja-jp/windows/package-manager/winget/) 経由で自動インストールされます（Windows 10/11標準搭載。古い環境で winget が無い場合は下記のリンクから手動インストールしてください）。

1. このプロジェクトのフォルダを開き、**`start.bat` をダブルクリック**します。
2. 「WindowsによってPCが保護されました」と出た場合は「詳細情報」→「実行」を選んでください（初回のみ）。
3. 自動的にブラウザが開き、アプリが使える状態になります。

手動でインストールしたい場合：[Python](https://www.python.org/downloads/)（インストール時に「Add python.exe to PATH」に必ずチェック）／[Node.js](https://nodejs.org/)（LTS版）／ffmpeg（`winget install ffmpeg`）。

### 終了するには

開いたウィンドウを閉じるか、ウィンドウ内で `Ctrl+C` を押してください。バックエンド・フロントエンドの両方が自動的に終了します。

> これらのスクリプトは、初回のみ自動でセットアップ（前提ツールの導入・仮想環境作成・依存関係インストール）を行い、2回目以降は数秒でアプリが起動します。手動でのセットアップ手順を知りたい方や、開発者の方は以降の章を参照してください。
